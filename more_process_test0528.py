    ...
    # 一对一匹配
        ...
        if df_b.empty and not df_a.empty:
            for index_a in df_a.index:
                print(f"a表第{index_a + 2}行在97524 SAP勘定明細表中没有匹配")
            return df_a2, df_b2
        ...
        for index_b, row_b in df_b.iterrows():
            ...
            if amount_b in hash_a:
                ...
                if len(matched_rows_a) == 0:
                    ...
                    df_a2.loc[int(int(macheds[0]) - 2), "匹配结果"] = "2对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[1]) - 2), "匹配结果"] = "2对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
        ...
        for index_b, row_b in df_b.iterrows():
            ...
            if amount_b in hash_a:
                ...
                if len(matched_rows_a) == 0:
                    ...
                    macheds = [str(i + 2) for i in matched_rows_a]
                    df_a2.loc[int(int(macheds[0]) - 2), "匹配结果"] = "3对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[1]) - 2), "匹配结果"] = "3对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[2]) - 2), "匹配结果"] = "3对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
        ...
        for index_b, row_b in df_b.iterrows():
            ...
            if amount_b in hash_a:
                ...
                if len(matched_rows_a) == 0:
                    ...
                    macheds = [str(i + 2) for i in matched_rows_a]
                    df_a2.loc[int(int(macheds[0]) - 2), "匹配结果"] = "4对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[1]) - 2), "匹配结果"] = "4对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[2]) - 2), "匹配结果"] = "4对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[3]) - 2), "匹配结果"] = "4对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
        ...
        for index_a, row_a in df_a.iterrows():
            ...
            if amount_a in hash_b:
                ...
                if len(matched_rows_b) == 0:
                    ...
                    macheds = [str(i + 2) for i in matched_rows_b]
                    matched_rows_b_str = ", ".join(macheds)
                    df_b2.loc[int(macheds[0]) - 2, "匹配结果"] = "5对一匹配：与残高照合表第" + str(index_a + 12) + "行匹配"
                    df_b2.loc[int(macheds[1]) - 2, "匹配结果"] = "5对一匹配：与残高照合表第" + str(index_a + 12) + "行匹配"
                    df_b2.loc[int(macheds[2]) - 2, "匹配结果"] = "5对一匹配：与残高照合表第" + str(index_a + 12) + "行匹配"
                    df_b2.loc[int(macheds[3]) - 2, "匹配结果"] = "5对一匹配：与残高照合表第" + str(index_a + 12) + "行匹配"
                    df_b2.loc[int(macheds[4]) - 2, "匹配结果"] = "5对一匹配：与残高照合表第" + str(index_a + 12) + "行匹配"
                    df_a2.loc[index_a, "匹配结果"] = (f"一对5匹配：与97524 SAP勘定明細表第{matched_rows_b_str}行匹配")
                    # print(f"一对四匹配：a表第{index_a + 2}行与97524 SAP勘定明細表第{matched_rows_b_str}行匹配")
                    df_a.drop(index=index_a, inplace=True)
        # # 5对一匹配
        for index_b, row_b in df_b.iterrows():
            ...
            if amount_b in hash_a:
                ...
                if len(matched_rows_a) == 0:
                    ...
                    macheds = [str(i + 2) for i in matched_rows_a]
                    df_a2.loc[int(int(macheds[0]) - 2), "匹配结果"] = "5对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[1]) - 2), "匹配结果"] = "5对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[2]) - 2), "匹配结果"] = "5对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[3]) - 2), "匹配结果"] = "5对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_a2.loc[int(int(macheds[4]) - 2), "匹配结果"] = "5对一匹配：与97524 SAP勘定明細表第" + str(index_b + 2) + "行匹配"
                    df_b2.loc[index_b, "匹配结果"] = (f"一对5匹配：与残高照合表第{macheds}行匹配")
                    matched_rows_a_str = ", ".join(macheds)
                    # print(f"四对一匹配：a表第{matched_rows_a_str}行与97524 SAP勘定明細表第{index_b + 2}行匹配")
                    df_b.drop(index=index_b, inplace=True)
...
def process_rows_parallel(df_a, df_b):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers_strs) as executor:
        results = [
...
if __name__ == '__main__':
    ...
    max_workers_strs = os.cpu_count()  # 获取CPU核心数
    print("开始运行5对5的匹配，多进程:"+str(max_workers_strs))
    x = Run_Files()
